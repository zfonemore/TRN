...
class VideoMultiScaleMaskedTransformerDecoder_frame_tarvis(VideoMultiScaleMaskedTransformerDecoder):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            sem_classification=True,
        )

        # learnable query features
        self.sem_query_feat = nn.Embedding(num_classes+1, hidden_dim)
        # learnable query p.e.
        self.sem_query_embed = nn.Embedding(num_classes+1, hidden_dim)
    ...
    def forward(self, x, mask_features, mask = None):
        ...
        sem_query_embed = self.sem_query_embed.weight.unsqueeze(1).repeat(1, bs, 1)
        sem_output = self.sem_query_feat.weight.unsqueeze(1).repeat(1, bs, 1)

        query_embed = torch.cat((query_embed, sem_query_embed))
        output = torch.cat((output, sem_output))
        ...
        pred_embds = self.decoder_norm(output[:100])
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size):
        ...
        outputs_class = torch.einsum("bqc,bsc->bqs", decoder_output[:, :100], decoder_output[:, 100:])
        ...
        return outputs_class, outputs_mask[:, :100], attn_mask

...
class VideoMaskFormer_frame_mem(nn.Module):
    """
    ...
    def post_processing(self, outputs):
        pred_logits, pred_masks, pred_embds, pred_add = outputs['pred_logits'], outputs['pred_masks'], outputs['pred_embds'], outputs['pred_add']

        # nc number of clip, ct frame of clip
        # pred_logits: 1 nc q c
        # pred_masks: 1 q nc ct h w
        pred_logits = pred_logits[0]
        pred_masks = einops.rearrange(pred_masks[0], 'q nc ct h w -> nc q ct h w')
        pred_embds = einops.rearrange(pred_embds[0], 'c nc q -> nc q c')
        ...
        out_masks = torch.stack(out_masks, dim=1)  # q ct h w -> q nc ct h w
        out_masks = einops.rearrange(out_masks, 'q nc ct h w -> q (nc ct) h w')
        t = 2
        if pred_add > 0:
            out_masks = out_masks[:, :-(t-pred_add)]
